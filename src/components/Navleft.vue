<template>
  <div class=" w-80 h-screen fixed top-20 ">
    <div class="bg-white h-full w-full relative">
      ...
      <!-- 导航 -->
      <div class=" mb-8">
        <div class="text-xs w-full p-2 shadow-bottom Cannotbeobtained">导航</div>
        <ul class="text-1.5xl font-normal ">
          <router-link tag="li" v-for="(item, index) in navData.Navigation" :key="index" :to="item.url" class="flex text-gray-700 hover:bg-gray-200 py-4 transition-all cursor-pointer">
            <img :src="item.src" class="mr-7 ml-7" width="20" />
            <span>{{ item.name }}</span>
          </router-link>
        </ul>
      </div>
      <!-- 组成 -->
      <div class=" mb-8">
        <div class="text-xs w-full p-2 shadow-bottom Cannotbeobtained">组成</div>
        <ul class="text-1.5xl font-normal">
      ...
      <!-- 底部导航 -->
      <div class="fixed bottom-0 flex flex-nowrap w-80 p-1 text-xs my-2 shadow-top ">
